...
test('loads the synthetic v10.33 release', async ({ page }) => {
  await page.goto('/');
  await expect(page).toHaveTitle(/Beaufort Learning Harbor v10\.33/);
  await expect(page.locator('html')).toHaveAttribute('data-demo-build', 'synthetic');
  await expect(page.locator('html')).toHaveAttribute('data-release', 'v10.33');
