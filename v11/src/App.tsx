    ...
    <div className="page-stack">
        ...
        <div>
          ...
          <p>
            {student
              ...
              : 'The new foundation connects planning, delivery, evidence, support, and records without turning every capability into a separate destination.'}
          </p>
        </div>
        <div className="hero-badge"><strong>v11</strong><span>Foundation alpha 1</span></div>
      </section>
      ...
      <section className="metric-grid" aria-label="Foundation readiness">
        ...
        <article className="metric-card">
          ...
          <p>{runtimeConfiguration.mode === 'cloud' ? 'Database policies enforce access.' : 'No real account or family data is being used.'}</p>
        </article>
      ...
      <section className="panel workflow-panel">
        <div className="section-heading">
          <div><span className="eyebrow">Primary workflow</span><h2>{student ? 'Today’s learning path' : 'The household operating path'}</h2></div>
          <span className="status-chip neutral">Designed before expanded</span>
        </div>
        <div className="workflow-steps">
          {steps.map((step, index) => <div className="workflow-step" key={step}><span>{index + 1}</span><strong>{step}</strong></div>)}
        </div>
  ...
  return (
    <div className="page-stack">
      <section className="page-heading">
        ...
        <p>Permissions follow real relationships rather than assuming every adult can see every learner.</p>
      </section>
      <section className="model-grid">
        <article className="panel model-card"><span className="model-number">01</span><h2>Account</h2><p>Supabase Auth owns credentials, recovery, and sessions. The application never stores passwords.</p></article>
        ...
        <article className="panel model-card"><span className="model-number">03</span><h2>Group</h2><p>Teachers, directors, and administrators receive only the permissions required for their work.</p></article>
      </section>
  ...
  return (
    <div className="page-stack">
      <section className="page-heading">
        ...
        <p>The preview works without a third-party account. Real cloud configuration begins only after owner-controlled setup.</p>
      </section>
      <section className="settings-grid">
        <article className="panel connection-card">
          <div className="connection-heading"><span className="connection-icon">S</span><div><h2>Supabase</h2><p>Authentication, Postgres, row-level security, and active file metadata.</p></div></div>
          <span className={`status-chip ${runtimeConfiguration.supabaseConfigured ? 'resolved' : 'neutral'}`}>{runtimeConfiguration.supabaseConfigured ? 'Configured' : 'Not connected'}</span>
          <p className="muted">Only the public URL and publishable key belong in browser configuration. Service-role keys remain server-side.</p>
        </article>
        <article className="panel connection-card">
          <div className="connection-heading"><span className="connection-icon">CF</span><div><h2>Cloudflare</h2><p>React assets and the Worker API deploy as one preview unit.</p></div></div>
          <span className="status-chip acknowledged">Preview isolated</span>
          <p className="muted">The preview Worker name cannot overwrite the v10.43 production Worker.</p>
        </article>
        <article className="panel connection-card">
          <div className="connection-heading"><span className="connection-icon">B</span><div><h2>BAND</h2><p>Optional channel for reviewed announcements and reminders.</p></div></div>
      ...
      <section className="panel permission-summary">
        <h2>Current access</h2><p>{definition.description}</p>
        <div className="permission-chips">{definition.capabilities.map((capability) => <span key={capability}>{capability}</span>)}</div>
        {!hasCapability(role, 'manage-group-settings') && <p className="muted">Group configuration controls are hidden for this role.</p>}
        {hasCapability(role, 'manage-system-settings') && <p className="message success">System administration preview is enabled.</p>}
      </section>
    </div>
  );
}
...
export default function App() {
  ...
  const actor: SupportActor = cloud.identity
    ? { id: cloud.identity.userId, label: cloud.identity.label, role, organizationId: cloud.identity.organizationId }
    : { id: `preview-${role}`, label: `Preview ${getRoleDefinition(role).label}`, role, organizationId: 'preview-organization' };
  const repository: SupportRepository = cloud.identity && cloudSupportRepository ? cloudSupportRepository : localSupportRepository;
  const visibleNav = NAV_ITEMS.filter((item) => item.id !== 'group' || hasCapability(role, 'view-group'));
    ...
    <div className="app-shell" data-testid="app-shell">
      <aside className="sidebar">
        ...
        <nav className="primary-nav" aria-label="Main navigation">
          {visibleNav.map((item) => (
            <button type="button" key={item.id} className={screen === item.id ? 'active' : ''} onClick={() => navigate(item.id)} data-testid={`nav-${item.id}`}>
              <span className="nav-icon" aria-hidden="true">{item.icon}</span><span><strong>{item.label}</strong><small>{item.description}</small></span>
            </button>
          ))}
        </nav>
        ...
        <header className="topbar">
          ...
          <div className="identity-controls">
            {runtimeConfiguration.mode === 'local-preview' ? (
              <label className="role-preview-control"><span>Preview role</span><select value={previewRole} onChange={(event) => setPreviewRole(event.target.value as AppRole)} data-testid="role-select">{APP_ROLES.map((option) => <option key={option} value={option}>{getRoleDefinition(option).label}</option>)}</select></label>
            ) : (
              <><div className="identity-label"><strong>{cloud.identity?.label}</strong><span>{getRoleDefinition(role).label}</span></div><button className="button ghost small" type="button" onClick={() => void cloud.signOut()}>Sign out</button></>
            )}
          </div>
        </header>
      ...
      <nav className="mobile-nav" aria-label="Mobile navigation">
        {visibleNav.map((item) => <button type="button" key={item.id} className={screen === item.id ? 'active' : ''} onClick={() => navigate(item.id)}><span aria-hidden="true">{item.icon}</span><small>{item.label}</small></button>)}
      </nav>
    </div>
  );
}
