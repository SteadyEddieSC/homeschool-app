...
const migration = JSON.parse(await readFile(path.join(outputDirectory, 'migration-rehearsal-report.json'), 'utf8'));
if (migration.release !== '11.0.0-rc.1' || migration.syntheticOnly !== true || migration.dryRunWrites !== 0) throw new Error('Production readiness requires the validated synthetic rc.1 migration report.');
if (migration.safeguards?.liveMigrationEnabled !== false || migration.safeguards?.productionCutover !== false) throw new Error('rc.1 must keep live migration and cutover disabled.');
...
const report = {
  ...
  evidenceDigests: { source: migration.sourceDigest, plan: migration.planDigest },
  exclusions: ['learner names and work', 'credentials and authentication state', 'provider configuration values', 'raw error text and queue payloads']
};
if (!report.automatedEvidence.every((item) => item.passed)) throw new Error('One or more rc.1 automated readiness gates failed.');
if (report.productionReady !== false || report.effectiveDecision !== 'not-ready' || report.automatedPromotionAllowed !== false) throw new Error('CI must never declare or automate production readiness.');
const serialized = JSON.stringify(report);
for (const forbidden of ['Synthetic Learner','Synthetic Harbor Household','sb_secret_','service_role','access_token']) if (serialized.includes(forbidden)) throw new Error(`Readiness report contains forbidden material: ${forbidden}`);
await mkdir(outputDirectory, { recursive: true });
await writeFile(path.join(outputDirectory, 'production-readiness-report.json'), `${JSON.stringify(report, null, 2)}\n`);
