...
const requiredEnvironment = ['VITE_SUPABASE_URL','VITE_SUPABASE_PUBLISHABLE_KEY','V11_PREVIEW_URL','CLOUDFLARE_ACCOUNT_ID','CLOUDFLARE_API_TOKEN'];
function jwtRole(value) { try { const payload = value.split('.')[1]; if (!payload) return ''; return JSON.parse(Buffer.from(payload.replace(/-/g,'+').replace(/_/g,'/'),'base64').toString('utf8')).role ?? ''; } catch { return ''; } }
function present(name) { return String(process.env[name] ?? '').trim().length > 0; }
...
const report = {
  schema: 'beaufort-learning-harbor-hosted-pilot-doctor-v1', release: '11.0.0-rc.1', checkedAt: new Date().toISOString(),
...
if (unsafe.length) process.exitCode = 1; else if (missing.length) process.exitCode = 2;
